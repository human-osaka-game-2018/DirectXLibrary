...
{
	const float THRESHOLD_VALUE = 14.f;

	const float THRESHOLD_VALUE_DOWN = 16.f;

	const float THRESHOLD_VALUE_UP = -12.5f;

	switch (Direction)
	{
	case UP_DIRECTION:
		if (m_GyroSensor.y < THRESHOLD_VALUE_UP) return true;

		break;

	case DOWN_DIRECTION:
		if (m_GyroSensor.y > THRESHOLD_VALUE_DOWN) return true;
